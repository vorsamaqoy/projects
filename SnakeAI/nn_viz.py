        ...
        for layer, num_nodes in enumerate(layer_nodes):
            ...
            for node in range(num_nodes):
                ...
                # Output layer
                    ...
                    text = ('U', 'D', 'L', 'R')[node]  # Text representing the output

                    # Calculate positions
                    x_pos = h_offset + 30
                    y_pos = node * (radius * 2 + vertical_space) + v_offset + 1.5 * radius
                    ...
                    painter.drawText(x_pos, y_pos, str(text))  # Draw text

                    # Set the brush color based on which node is max
                    if node == max_out:
                        painter.setBrush(QtGui.QBrush(Qt.green))
                    else:
                        painter.setBrush(QtGui.QBrush(Qt.white))
        ...
        for l in range(1, len(layer_nodes)):
            ...
            for prev_node in range(prev_nodes):
                ...
                for curr_node in range(curr_nodes):
                    ...
                    # Ensure we convert all coordinates to integers
